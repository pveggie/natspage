...
RSpec.describe Photo, type: :model, focus: true do
  ...
  it "has a valid factory for uploading from local storate" do
  ...
  it "has a valid factory for uploading from url" do
  ...
  describe "ActiveModel validations", focus: false do
    ...
    it "is valid with no caption" do
      expect(build(:local_photo, caption_title: nil, caption_description: nil))
        .to be_valid
    end
    ...
    it {
      expect(local_photo).to validate_length_of(:caption_title)
        .is_at_most(60)
    }

    it {
      expect(local_photo).to validate_length_of(:caption_description)
        .is_at_most(300)
    }
  end
  ...
  describe "Cloudinary storage", focus: false do
    it "saves an uploaded photo in Cloudinary" do
      public_id = saved_photo.image_location.metadata["public_id"]
      expect(Cloudinary::Api.resources_by_ids(public_id)["resources"].count)
        .to eql(1)
    end
    ...
    it "tags images with natpage in Cloudinary" do
      ...
      last_with_tag =
        Cloudinary::Api
        .resources_by_tag("natpage", max_results: 1)["resources"][0]
  ...
  describe "ActiveRecord associations" do
    ...
    it {
      expect(local_photo)
        .to have_many(:category_entries).dependent(:destroy)
    }

    it {
      expect(local_photo)
        .to have_many(:categories).through(:category_entries)
    }
  end
  ...
  describe "callbacks" do
    ...
    it {
      expect(saved_photo)
        .to callback(:delete_from_cloudinary).after(:destroy)
    }
  end
